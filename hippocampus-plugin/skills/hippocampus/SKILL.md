...
- **Identity context**: See [references/identity.md](references/identity.md) when creating contracts, invoices, or formal documents
- **JIT update**: If identity.md is empty or contains placeholder text, prompt user to fill it before proceeding with document creation
...
- **Project** (`~/.claude/hippocampus/project/{name}/`): Client-specific content
  - Phantom nodes: Client-based (e.g., `[[client-acme]]`, `[[client-example]]`)
...
### Step 4: Publishing Decision (Optional)
If publishing workflow is configured (see README):
AskUserQuestion:
- **Publish to GitHub Pages?**
- Options: Yes / No
- If Yes: Add `publish: true` to frontmatter
- Result: File will be mirrored to public repo on push
